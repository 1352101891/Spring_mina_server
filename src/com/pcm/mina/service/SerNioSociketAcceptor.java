...
import static com.pcm.util.Constants.HB_INTERVAL;
import static com.pcm.util.Constants.TIME_IDLE;
   ...
   //创建bind()方法接收连接
    ...
    {	
       ...
       //getFilterChain() 获取 I/O 过滤器链，可以对 I/O 过滤器进行管理，包括添加和删除 I/O 过滤器。    	
        ...
        kaf.setRequestInterval(HB_INTERVAL);  //本服务器为被定型心跳  即需要每30秒接受一个心跳请求  否则该连接进入空闲状态 并且发出idled方法回调
        acceptor.getFilterChain().addLast("heart", kaf); 
        //读写通道60秒内无操作进入空闲状态
        acceptor.getSessionConfig().setIdleTime(IdleStatus.BOTH_IDLE, TIME_IDLE);
        //绑定逻辑处理器
