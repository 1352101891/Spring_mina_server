...
public class ServiceMainHandler extends IoHandlerAdapter{
   ...
   //	session.close(); //发送成功后主动断开与客户端的连接 实现短连接
	...
	public void sessionIdle(IoSession session, IdleStatus status) throws Exception {
		...
		PcmSession pcmSession=sessionManager.getSeesion(session);
		if (pcmSession!=null){
			if (pcmSession.getHeartbeat()<=0){
				pcmSession.setHeartbeat(System.currentTimeMillis());
			}else {
				long interval= System.currentTimeMillis()-pcmSession.getHeartbeat();
				if (interval>Constants.TIME_IDLE*2000){
					sessionClosed(session);//超过两次空闲，关闭连接
				}
			}
		}
	}
